...
function indekseGoreFenomen(dizi, index) {
  const profile = dizi[index];
  return `${index}. indekste bulunan fenomen: ${profile}`;
}
//console.log(fenomenler,3)
...
function profilListesi(fenomenler,i) {
  const isimler = [...fenomenler];
...
function fenomenEkle(dizi, number, profile, followers, posts, platform) {
  ...
  kopyaFenomenler.push(newObject);
...
function platformaGoreCokGonderiYapanFenomen(dizi,platform) {
    ...
    for (let i = 0; i < dizi.length; i++) {
      ...
      if (fenomen.platform === platform && fenomen.posts !== '' && fenomen.posts > maxPosts) {
        maxPosts = fenomen.posts;
        maxFenomen = fenomen.profile;
      }
    }
    return maxFenomen;
  }
